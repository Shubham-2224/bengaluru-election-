    ...
    def preprocess_image(self, image: Image.Image, for_ocr: bool = True, skip_heavy_ops: bool = False) -> Image.Image:
        ...
        if for_ocr:
            try:
                ...
                # WATERMARK SUPPRESSION: Force light-gray pixels to pure white.
                # Aggressive threshold (120-255) ensures even faint watermarks are eliminated.
                image = image.point(lambda p: 255 if p > 120 else p)
                ...
                # 6. ULTRA-STRICT ADAPTIVE THRESHOLDING (Binarization)
                if CV2_AVAILABLE:
                     img_np = np.array(image)
                     # Adaptive Threshold: Block Size 11, C=25 (Maximum cleanup for watermarks)
                     # Increasing C to 25 forces even moderately dark gray pixels to background.
